 ...
 *
 * $Log: configure.cpp,v $
 * Revision 1.3  2006/08/23 06:41:44  rcolobong
 * *** empty log message ***
 *
 * Revision 1.2  2006/08/17 09:24:28  rcolobong
 * *** empty log message ***
 *
...
class Feature
{
  public:
    ...
    struct CheckVersionInfo {
      CheckVersionInfo() : found(false) { }

      bool Locate(const string& testdir );
      bool CheckValue( const std::string& format, const string& value );
      bool ExtractVersion( const string& line, const std::string& type, string& value );

      bool found;
      string filename;
      string majorRangeValue;
      string minorRangeValue;
      string buildRangeValue;
    };

    list<CheckFileInfo> checkFiles;
    list<CheckVersionInfo> checkVersion;
...
void Feature::Parse(const string & optionName, const string & optionValue)
{
  ...
  else if (optionName == "CHECK_VERSION")
  {
    int comma = optionValue.find(',');
    if (comma == string::npos)
      return;

    CheckVersionInfo check;
    
    check.filename.assign( optionValue, 0, comma );

    int index = comma;
    comma = optionValue.find( ',', comma + 1);
    if (comma == std::string::npos)
      return;

    check.majorRangeValue.assign( optionValue, index + 1, comma - index - 1 );
    ...
    check.minorRangeValue.assign( optionValue, index + 1, comma - index - 1 );

    check.buildRangeValue.assign( optionValue, comma + 1, INT_MAX );
    if( check.buildRangeValue.empty() )
      return;

    checkVersion.push_back( check );
  }
...
bool Feature::Locate(const char * testDir)
{
  ...
  {
    for( list<CheckVersionInfo>::iterator version = checkVersion.begin();
      version != checkVersion.end();
      ++version )
    {
      if( !version->Locate(testDirectory) )
      {
        // set found into false in CheckFileInfo
        for( list<CheckFileInfo>::iterator file1 = checkFiles.begin();
          file1 != checkFiles.end(); ++file1 )
        {
          file1->found = false;
        }
...
void Feature::Trim( string& str )
{
	int trimPos;
	if ( ( trimPos = str.find_first_not_of( ' ' ) ) > 0 )
		str = str.substr( trimPos );
	if ( ( trimPos = str.find_last_not_of( ' ' ) ) != -1 )
		str = str.substr( 0, trimPos + 1 );
}
...
bool Feature::CheckVersionInfo::Locate( const string & testDirectory )
{
  string path = testDirectory + filename;
  ifstream file(path.c_str(), ios::in);
  if( !file.is_open() )
    return false;

  std::string majorValue;
  std::string minorValue;
  ...
  {
    if( majorValue.empty() )
      ExtractVersion( line, "MAJOR_VERSION", majorValue );

    if( minorValue.empty() )
      ExtractVersion( line, "MINOR_VERSION", minorValue );
    
    if( buildValue.empty() )
      ExtractVersion( line, "BUILD_NUMBER", buildValue );
  ...
  if( !CheckValue( majorRangeValue, majorValue ) )
    return false;

  if( !CheckValue( minorRangeValue, minorValue ) )
    return false;

  if( !CheckValue( buildRangeValue, buildValue ) )
    return false;

  return true;
}

bool Feature::CheckVersionInfo::CheckValue( const string& format, const string& value )
{
  if( value == "*" )
    return true;
  ...
  {
    std::string minimum = format.substr( 0, index );
    Feature::Trim( minimum );
    if( atoi( value.c_str() ) <= atoi( minimum.c_str() ) ) 
      return true;
  }
  ...
  {
    std::string minimum = format.substr( 0, index );
    std::string maximum = format.substr( index + 1, format.size() );
    Feature::Trim( minimum );
    Feature::Trim( maximum );

    int intValue = atoi( value.c_str() );
    if( intValue >= atoi( minimum.c_str() ) &&intValue <= atoi( maximum.c_str() ) )
      return true;
  }

  // check if value is equal to format
  if( format == value )
    return true;
...
bool Feature::CheckVersionInfo::ExtractVersion( const string& line, const string& type, string& value )
{
  std::size_t index = line.find( type );
  if( index != string::npos )
  {
    value = line.substr( index + type.size(), line.size() );
    Feature::Trim( value );
