 ...
 *
 * $Log: dtmf.cxx,v $
 * Revision 1.3  2006/12/12 23:53:47  joegenbaclor
 * More work on DTMF encoding
 *
 * Revision 1.2  2006/12/12 05:26:16  joegenbaclor
...
double PDTMFEncoder::sinetab[SINELEN];
...
void PDTMFEncoder::AddTone(double f1, double f2, unsigned ms)
{
  ...
  for (int n = 0; n < ns; n++) { 
    ...
    //double var1 = (double)2 * (double)M_PI * (double)ptr1 / (double)SAMPLERATE; 
	  //double var2 = (double)2 * (double)M_PI * (double)ptr2 / (double)SAMPLERATE;
    ...
    (static_cast<PBYTEArray&>(*this))[dataPtr++] = (BYTE)(ival & 0xff);
    (static_cast<PBYTEArray&>(*this))[dataPtr++] = (BYTE)(ival >> 8);
