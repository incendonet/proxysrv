    ...
    if (ignoreOtherSources) {
      ...
      if (!remoteAddress.IsValid()) {
        ...
        PTRACE(3, "RTP\tSet remote address from first " << channelName
               << " PDU from " << addr << ':' << port << " XOR=" << xorHash );
      }
      if (fromDataChannel) {
        if (remoteDataPort == 0)
          remoteDataPort = port;
...
BOOL RTP_UDP::WriteControl(RTP_ControlFrame & frame)
{
  ...
  PINDEX compoundSize = frame.GetCompoundSize();
  void * bytesToWrite = frame.GetPointer();
#if 1
  PBYTEArray out;
  if( xorHash )
  ...
  while (!controlSocket->WriteTo( bytesToWrite, compoundSize,
