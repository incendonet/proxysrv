...
namespace MD5
{
  class A2Hash : public MD5Hash
  {
    PCLASSINFO( A2Hash, PObject );
  public:
    A2Hash();
    ...
    BOOL operator==( const A2Hash & digest );

    A2Hash & operator=( const A2Hash & digest );

    PINLINE const PString & GetMethod()const{ return m_Method; };
    PINLINE void SetMethod( const PString & method ){ m_Method = method; };
    PINLINE const PString & GetURI()const{ return m_URI; };
    PINLINE void SetURI( const PString & uri ){ m_URI = uri; };
