...
#ifndef SDPPROTOCOLVERSION_H
#define SDPPROTOCOLVERSION_H

#include "SDPHeader.h"

namespace SDP
{
  class SDPProtocolVersion : public SDPHeader
  {
    PCLASSINFO( SDPProtocolVersion, SDPHeader );
  public:
    SDPProtocolVersion();
    ...
    SDPProtocolVersion & operator=( const PString & header );

    SDPProtocolVersion & operator=( const SDPProtocolVersion & header );

    SDPProtocolVersion & operator=( WORD version );

    BOOL operator==(WORD version);
    ...
    PINLINE WORD GetVersion(){ return m_Version; };
