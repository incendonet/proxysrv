...
namespace MD5
{
  class A1Hash : public MD5Hash
  {
    PCLASSINFO( A1Hash, PObject );
  public:
    A1Hash();
    ...
    BOOL operator==( const A1Hash & digest );

    A1Hash & operator=( const A1Hash & digest );

    PINLINE const PString & GetUserName()const{ return m_UserName; };
    PINLINE void SetUserName( const PString & userName ){ m_UserName = userName; };
