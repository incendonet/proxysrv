 ...
 *
 * $Log: RedirectSessionManager.h,v $
...
#ifndef REDIRECTSESSIONMANAGER_H
#define REDIRECTSESSIONMANAGER_H
...
namespace UACORE
{
  class RedirectSessionManager : public SIPSessionManager
  {
    PCLASSINFO( RedirectSessionManager, SIPSessionManager );
  public:


    RedirectSessionManager(
      SIPUserAgent & ua,
      PINDEX sessionThreadCount,
      PINDEX stackSize
    );

    virtual ~RedirectSessionManager();

    virtual SIPSession::GCRef OnCreateServerSession(
      const SIPMessage & request
    );

    virtual BOOL OnRequestRedirection(
      const SIPMessage & request,
